...
import { Loader2, Sparkles, Zap, Bot, Brain, CheckCircle2, ArrowRight, RefreshCw } from "lucide-react";
...
const ANALYZE_PHASES = [
  { name: "Reading jump context...", duration: 3, icon: Brain },
  { name: "Identifying automation patterns...", duration: 5, icon: Sparkles },
  { name: "Analyzing workflow potential...", duration: 6, icon: Zap },
  { name: "Generating opportunities...", duration: 4, icon: Bot },
  { name: "Finalizing analysis...", duration: 2, icon: CheckCircle2 },
];

const TOTAL_ESTIMATED_TIME = ANALYZE_PHASES.reduce((acc, phase) => acc + phase.duration, 0);

export function AnalyzeButton({ isAnalyzing, hasAnalysis, onAnalyze, disabled }: AnalyzeButtonProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < ANALYZE_PHASES.length; i++) {
      accumulatedTime += ANALYZE_PHASES[i].duration;
    ...
    setCurrentPhaseIndex(ANALYZE_PHASES.length - 1);
  ...
  const currentPhase = ANALYZE_PHASES[currentPhaseIndex];
    ...
    return (
      <div className="w-full space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="relative flex items-center justify-between w-full text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground font-medium">
                  Phase {currentPhaseIndex + 1} of {ANALYZE_PHASES.length}
                </span>
              </div>
        ...
        <p className="text-center text-xs text-muted-foreground">
          Estimated time remaining: ~{Math.max(TOTAL_ESTIMATED_TIME - elapsedSeconds, 2)} seconds
        </p>
      </div>
    );
  }

  // Already Analyzed - Just show subtle re-analyze button
  if (hasAnalysis) {
    return (
      <button
      ...
      >
        ...
        <div className="absolute -inset-0.5 bg-gradient-to-r from-muted/20 via-border/30 to-muted/20 rounded-2xl blur-md opacity-0 group-hover:opacity-50 transition duration-500" />
        ...
        <div className={cn(
          ...
          "border border-border/60 group-hover:border-primary/40",
          ...
          <RefreshCw className="relative w-4 h-4 text-muted-foreground group-hover:text-primary transition-colors" />
  ...
  // Idle State - Premium liquid glass button (like AgentBuildButton)
  return (
      ...
      >
        ...
        <div className={cn(
          ...
          <div className="relative p-2 rounded-xl bg-primary/10 group-hover:bg-primary/20 transition-colors">
            <Sparkles className="h-5 w-5 text-primary group-hover:text-primary transition-colors" />
          </div>
